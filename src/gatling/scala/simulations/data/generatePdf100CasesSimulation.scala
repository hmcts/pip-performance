...
class generatePdf100CasesSimulation extends Simulation {

  private val generatePdfExec = scenario("Generate PDF 100 Cases")
    .exec(OAuthAPI.authData) // authenticate once per user
    .exec(PublicationScenarios.generatePdfHundredCases)


  setUp(
    generatePdfExec.inject(
