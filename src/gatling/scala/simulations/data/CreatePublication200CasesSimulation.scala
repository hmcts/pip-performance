...
class CreatePublication200CasesSimulation extends Simulation {
  ...
  private val createPublicationDifferentSizesExec = scenario("Create Publication Civil And Family request 200 case")
    .exec(OAuthAPI.authData) // authenticate once per user
    .exec(PublicationScenarios.createTwoHundredCases)
