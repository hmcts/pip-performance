...
class generateArtefactSummary2CasesSimulation extends Simulation {

  private val generateArtefactSummaryExec = scenario("Generate Artefact Summary 2 Cases")
    .exec(OAuthAPI.authData) // authenticate once per user
    .exec(PublicationScenarios.generateArtefactTwoCases)
