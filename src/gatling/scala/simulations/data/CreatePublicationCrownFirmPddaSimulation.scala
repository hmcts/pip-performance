...
class CreatePublicationCrownFirmPddaSimulation extends Simulation {
  ...
  private val createPublicationCrownFirmPddaSExec = scenario("Create Publication Crown Firm Pdda")
    .exec(OAuthAPI.authData) // authenticate once per user
    .exec(PublicationScenarios.createPublicationCrownFirmPdda)


  setUp(
    createPublicationCrownFirmPddaSExec.inject(
