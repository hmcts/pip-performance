...
class generateArtefactSummary50CasesSimulation extends Simulation {

  private val generateArtefactSummaryExec = scenario("Generate Artefact Summary 50 Cases")
    .exec(OAuthAPI.authData) // authenticate once per user
    .exec(PublicationScenarios.generateArtefactFiftyCases)


  setUp(
    generateArtefactSummaryExec.inject(
